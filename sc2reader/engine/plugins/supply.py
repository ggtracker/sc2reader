...
class SupplyTracker(object):
    def add_to_units_alive(self,event,replay):
        ...
        if unit_name in  self.unit_name_to_supply:
            ...
            replay.players[event.control_pid-1].current_food_used[time_built]= total_supply
            print("Second",time_built,replay.players[event.control_pid-1],"SUPPLY",replay.players[event.control_pid-1].current_food_used[time_built])
           ...
           ## see if the unit provides supply
            ...
            replay.players[event.control_pid-1].current_food_made[time_complete]= total_supply_gen
            print("Second",time_complete, replay.players[event.control_pid-1],"Built",replay.players[event.control_pid-1].current_food_made[time_complete])
        else:
    ...
    def handleInitGame(self, event, replay):
        ...
        self.unit_name_to_supply = { 
            #Zerg
            "Drone":(1,17),"Zergling":(1,25),"Baneling":(0,20),"Queen":(2,50),\
            "Hydralisk":(2,33),"Roach":(2,27),"Infestor":(2,50),"Mutalisk":(2,33),\
            "Corruptor":(2,40),"Utralisk":(6,55),"Broodlord":(2,34),\
            "SwarmHost":(3,40), "Viper":(3,40),\
            #Terran
            "SCV":(1,17),"Marine":(1,25),"Marauder":(2,30),"SiegeTank":(2,45),\
            "Reaper":(1,45),"Ghost":(2,40),"Hellion":(2,30),"Thor":(6,60),\
            "Viking":(2,42),"Medivac":(2,42),"Raven":(2,60), "Banshee":(3,60),\
            "Battlecruiser":(6,90), "Hellbat":(2,30),"WidowMine":(2,40),\
            #Protoss
            "Probe":(1,17),"Zealot":(2,38),"Stalker":(2,42),"Sentry":(2,42),\
            "Observer":(1,30), "Immortal":(4,55),"WarpPrism":(2,50),\
            "Colossus":(6,75), "Phoenix":(2,35),"VoidRay":(4,60), \
            "HighTemplar":(2,55),"DarkTemplar":(2,55), "Archon":(4,12),\
            "Carrier":(6,120), "Mothership":(6,100),"MothershipCore":(2,30),\
            "Oracle":(3,50),"Tempest":(4,60)}
        
        self.supply_gen_unit = {
            #overlord build time is zero because event for units are made when 
            # it is born not when it's created
            "Overlord":(8,0),"Hatchery":(2,100), \
            "SupplyDepot":(8,30),"CommandCenter":(11,100),\
            "Pylon":(8,25),"Nexus":(10,100)
        }
    ...
    def handleEndGame(self, event, replay):
        for player in replay.players:
            player.current_food_used = sorted(player.current_food_used.iteritems(), key=lambda x: x[0])
            player.current_food_made = sorted(player.current_food_made.iteritems(), key=lambda x:x[0])
